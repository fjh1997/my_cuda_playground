...
void matrixcpy(float *src,float *dst, int size) {
...
int MatrixMultiply(int argc, char **argv, int block_size, const dim3 &dimsA,
  ...
  float s_A[size_A]={0.1,0.2,0.3,0.4};
  float s_B[size_B]={0.5,0.6,0.7,0.8};
  matrixcpy(s_A,h_A,size_A);
  matrixcpy(s_B,h_B,size_B);
  ...
  printf("MatrixC(%d,%d)\n", dimsC.x, dimsC.y);
  printmatrix(h_C,dimsC.x * dimsC.y);
